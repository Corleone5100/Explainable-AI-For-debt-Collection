"""
Graph Builder — Contagion Graph + GAT Training
================================================
Builds weighted borrower graph, trains GAT, extracts embeddings.
...
# -- Database Connection --
def get_db_engine():
    """Create database engine from config with error handling."""
    try:
...
def load_data(limit=None):
    ...
    logger.info(f"  Loaded {len(df)} borrowers")
...
def build_weighted_edges(df):
    logger.info("\n" + "=" * 60)
    logger.info("STEP 2: Building Weighted Edges (Contagion Links)")
    logger.info("=" * 60)
    ...
    for gidx, (name, group) in enumerate(tqdm(grouped, desc="Building edges", ncols=80)):
        ...
        if (gidx + 1) % 100 == 0:
            logger.info(f"    Processed {gidx + 1}/{total_groups} groups...")
    ...
    logger.info(f"  Created {len(unique_edges)} unique edges with weights")
    logger.info(f"  Weight range: [{min(unique_weights):.4f}, {max(unique_weights):.4f}]")
    logger.info(f"  Mean weight: {np.mean(unique_weights):.4f}")
...
def compute_structural_features(df, edge_index):
    logger.info("\n" + "=" * 60)
    logger.info("STEP 3: Computing Node Structural Features")
    logger.info("=" * 60)
    ...
    logger.info("  Building NetworkX graph for centrality metrics...")
    G = nx.Graph()
    ...
    if num_nodes > 5000:
        logger.info(f"  Computing betweenness centrality (sampled for {num_nodes} nodes)...")
    ...
    logger.info(f"  Degree:  [{df['node_degree'].min():.0f}, {df['node_degree'].max():.0f}] "
          f"(mean={df['node_degree'].mean():.2f})")
    logger.info(f"  PageRank: [{df['pagerank'].min():.6f}, {df['pagerank'].max():.6f}]")
    logger.info(f"  Betweenness: [{df['betweenness'].min():.6f}, {df['betweenness'].max():.6f}]")
...
def validate_graph_structure(df, G, edge_index):
    logger.info("\n" + "=" * 60)
    logger.info("STEP 4: Graph Validation Metrics")
    logger.info("=" * 60)
    ...
    logger.info(f"  Density: {2 * num_edges / (num_nodes * (num_nodes - 1)):.6f}")
    ...
    logger.info(f"  Clustering coefficient: {clustering:.4f}")
    ...
    try:
        ...
        logger.info("  Degree assortativity: could not compute (graph too small or disconnected)")

    # -- Risk Assortativity --
    logger.info("  Computing risk assortativity...")
    same_risk_edges = 0
    total_edges = 0
    for u, v in G.edges():
    ...
    logger.info(f"  Same-risk edge ratio: {risk_assort:.4f} ({same_risk_edges}/{total_edges})")

    # -- Degree Distribution Stats --
    degrees = [d for n, d in G.degree()]
    logger.info(f"  Degree distribution: min={min(degrees)}, max={max(degrees)}, "
...
def detect_communities(df, G):
    logger.info("\n" + "=" * 60)
    logger.info("STEP 5: Community Detection (Louvain)")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info(f"  Community risk %: [{df['community_risk_pct'].min():.1f}, "
              f"{df['community_risk_pct'].max():.1f}]")
        logger.info(f"  Community sizes: [{df['community_size'].min()}, "
              f"{df['community_size'].max()}]")

    except ImportError:
        logger.info("  python-louvain not installed. Skipping community detection.")
        logger.info("  Install with: pip install python-louvain")
        df['community_id'] = -1
...
def compute_multihop_signals(df, edge_index):
    logger.info("\n" + "=" * 60)
    logger.info("STEP 6: Multi-Hop Neighborhood Signals")
    logger.info("=" * 60)
    ...
    logger.info(f"  1-hop stress: mean={df['neighborhood_stress_1hop'].mean():.2f}, "
    ...
    logger.info(f"  2-hop stress: mean={df['neighborhood_stress_2hop'].mean():.2f}, "
    ...
    logger.info(f"  3-hop stress: mean={df['neighborhood_stress_3hop'].mean():.2f}, "
...
class GATWithEmbedding(GATNet):
    ...
    def forward(self, x, edge_index, edge_attr=None, return_attention=False):
        # Layer 1
        if return_attention:
            x, (attn_edge_index, attn_weights) = self.conv1(
                x, edge_index,
                edge_attr=edge_attr.view(-1, 1) if edge_attr is not None else None,
                return_attention_weights=True
            )
        else:
            x = self.conv1(x, edge_index, edge_attr=edge_attr.view(-1, 1) if edge_attr is not None else None)
            attn_edge_index, attn_weights = None, None
        ...
        if return_attention:
            return logits, embeddings, (attn_edge_index, attn_weights)
        return logits, embeddings


def train_gnn(data, num_classes, epochs=None, lr=None):
    if epochs is None:
        epochs = cfg.gnn_epochs
    if lr is None:
        lr = cfg.gnn_lr
    logger.info("\n" + "=" * 60)
    logger.info("STEP 7: Training GAT (Graph Attention Network)")
    logger.info("=" * 60)
    logger.info(f"  Architecture: GATConv(8->32, 4 heads) -> ReLU -> GATConv(128->16)")
    logger.info(f"  Epochs: {epochs}, LR: {lr}, Device: {device}")
    ...
    for epoch in range(epochs):
        optimizer.zero_grad()
        out, embeddings, attn = model(data.x, data.edge_index,
        ...
        if loss.item() < best_loss:
            ...
            # Capture attention weights from best epoch
            if attn is not None:
                best_attention = (attn[0].cpu().detach().clone(),
                                  attn[1].cpu().detach().clone())
    ...
    return model, best_embeddings, best_attention
...
def main():
    ...
    if gat_attention is not None:
        logger.info("\n  Saving GAT attention weights...")
        attn_edge_idx = gat_attention[0].numpy()
        attn_values = gat_attention[1].mean(dim=1).numpy()  # Average across heads

        attn_df = pd.DataFrame({
            'source_node': attn_edge_idx[0],
            'target_node': attn_edge_idx[1],
            'attention_weight': attn_values,
        })
        attn_df.to_csv("gat_attention_weights.csv", index=False)
        logger.info(f"  Saved {len(attn_df)} attention weights to gat_attention_weights.csv")

    # -- 11. Attach GNN Embeddings to DataFrame --
    ...
    logger.info("\n" + "=" * 60)
    logger.info("FINAL FEATURE SUMMARY")
    logger.info("=" * 60)
    ...
    logger.info(f"  Structural features:    {len(structural_features)}")
    logger.info(f"  Community features:     {len(community_features)}")
    logger.info(f"  Multi-hop signals:      {len(multihop_features)}")
    logger.info(f"  GAT embeddings:         {len(gat_feature_names)}")
    logger.info(f"  ---------------------------------")
    logger.info(f"  Total RL observation:   {len(original_features) + len(all_new_features)} dimensions")
